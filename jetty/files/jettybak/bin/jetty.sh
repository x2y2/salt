...
case "$ACTION" in
  start)
    ...
    then
      ...
      if [ "$JETTY_USER" ] && [ $(whoami) != 'bestpay' ] 
      then
        touch "$JETTY_PID"
        chown "$JETTY_USER" "$JETTY_PID"
        # FIXME: Broken solution: wordsplitting, pathname expansion, arbitrary command execution, etc.
        su - "$JETTY_USER" -c "
          exec nohup ${RUN_CMD[*]} --daemon >/dev/null 2>&1 &
          disown \$!
          echo \$! > '$JETTY_PID'"
      elif [ $(whoami) == 'bestpay' ];then
